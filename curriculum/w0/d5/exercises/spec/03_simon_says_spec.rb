...
# `echo` takes a parameter and 'echoes' it back to them.
#
# `shout` takes a parameter, and 'shouts' it back to them.
#
# `repeat` takes one OR two parameters, and 'repeats' the first parameter
# depending on the second parameter.
#
# `start_of_word(str, num)` returns the first `num` amount of letters of the `str`.
#
# `first_word(str)` returns the first word of the string
#
# `titleize(title)` returns the `title` with properly capitalized letters
